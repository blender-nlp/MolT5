...
parser.add_argument('--batch_size', type=int, default=116, help='batch size')
...
encoder = EncoderRNN(args, len(smiles_tokenizer), device)
decoder = DecoderRNN(args, len(tokenizer), device)
...
def train(smiles_tokens, lengths, text, text_mask, encoder, decoder, encoder_optimizer, decoder_optimizer):
    ...
    encoder_outputs, encoder_hidden = encoder(smiles_tokens, lengths)
    ...
    if use_teacher_forcing:
        for t in range(args.max_length):
            ...
            mask_loss, nTotal = maskNLLLoss(decoder_output, text[:,t], text_mask[:,t])
            loss += mask_loss
...
def eval(smiles_tokens, lengths, text, text_mask, encoder, decoder):
    ...
    inds = torch.stack(decoder_outputs, dim=1).argmax(dim=2)
...
torch.save(encoder.state_dict(), args.saved_path + 'rnn_smiles2caption_encoder_baseline_epoch' + str(args.epochs) + '.pt')
torch.save(decoder.state_dict(), args.saved_path + 'rnn_smiles2caption_decoder_baseline_epoch' + str(args.epochs) + '.pt')

with open(args.saved_path + 'rnn_smiles2caption_vocab_baseline_epoch' + str(args.epochs) + '.pkl', 'wb') as f:
...
with torch.no_grad():
    for j, d in enumerate(test_dataloader):
        ...
        if j % 10 == 0: print('Step:', j)

        smiles_tokens = d['smiles_tokens'].transpose(0,1)
        lengths = d['lengths']
